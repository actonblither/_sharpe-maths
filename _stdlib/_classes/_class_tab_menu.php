...
class _tab_menu extends _setup {
	...
	public function _make_tabs(){
		...
		for ($i=0; $i < count($this->_titles); $i++){
			...
			if ($this->_titles[$i] == '+'){$el_id = '_add_new';}else{$el_id = 'aa'.$i;}
			$this->str .= '<div class = "tab-container">';
			$this->_field_array = explode('::', $this->_fields[$i]);
			if ($el_id != '_add_new'){
				$this->str .= $this->_build_tab_btn($i);
			}else{
				$this->str .= "<button id = '_add_new' class = 'add_new tab_add_btn point ttip' title = '".rv($this->_help[$i])."' type = 'button'>+</button>";
			}
